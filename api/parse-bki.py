...
from obligation_merge import merge_uid_obligations
from kredit_info_adapter import parse_kredit_info
...
def parse_pdf(pdf_path: str) -> dict:
    ...
    if bureau == "nbki":
        ...
        obligations = merge_uid_obligations(contracts)
        return {"ok":True,"bureau":"НБКИ","bureau_code":"nbki","format_version":fmt,"page_count":page_count,
            ...
            "contracts":contracts,"obligations":obligations,"applications":applications,"queries":queries,
            "meta":{"uid_nonempty":sum(1 for r in contracts if r.get("uid")),"uid_unique":len({r["uid"] for r in contracts if r.get("uid")}),"obligation_count":len(obligations),"application_uid_nonempty":sum(1 for a in applications if a.get("uid")),"summary_counts":parsed.get("summary_counts",{}),"contract_qc":parsed.get("contract_qc",{})},
            "diagnostics":{"file_sha256":file_sha256,"python_version":platform.python_version(),"pymupdf_version":getattr(pymupdf,"__version__",None)},"warnings":parsed.get("warnings",[])}
    ...
    if bureau == "kredit_info":
        parsed = parse_kredit_info(pdf_path)
        contracts, applications, queries = parsed["contracts"], parsed["applications"], parsed["queries"]
        obligations = merge_uid_obligations(contracts)
        return {"ok":True,"bureau":"Кредит Инфо","bureau_code":"kredit_info","format_version":fmt,"page_count":page_count,
            "counts":{"contracts":len(contracts),"applications":len(applications),"queries":len(queries)},
            "contracts":contracts,"obligations":obligations,"applications":applications,"queries":queries,
            "meta":{**parsed.get("meta",{}),"obligation_count":len(obligations)},

...
Каждая карточка ограничена маркерами:
[[CONTRACT_BLOCK item_index=N]]
...
[[END_CONTRACT_BLOCK item_index=N]]

ОБЯЗАТЕЛЬНО:
- Верни ровно ОДИН объект на КАЖДЫЙ входной item_index.
- item_index перепиши без изменения.
- Даже если каких-то полей нет, объект всё равно должен присутствовать.
- Не возвращай пустой массив, если вход содержит карточки.
- Если карточка не является договором, всё равно верни объект с item_index и creditor=null, но warning="not_a_contract".
- Не создавай больше одного объекта на item_index.
- Собери полное название кредитора.
- "Идентификатор сделки" — это uid.
- Ничего не выдумывай.
...
Верни ТОЛЬКО JSON:
{
  ...
  "warnings": []
      ...
      task = `
Перед тобой набор УЖЕ ВЫДЕЛЕННЫХ строк таблицы кредитных заявлений Скоринг Бюро.
Каждая запись ограничена маркерами:
[[APPLICATION_ROW item_index=N]]
...
[[END_APPLICATION_ROW item_index=N]]

ОБЯЗАТЕЛЬНО:
- Верни ровно ОДИН объект на КАЖДЫЙ входной item_index.
- item_index перепиши без изменения.
- Не возвращай пустой массив, если во входе есть строки.
- Если поле не читается, оставь null, но объект не пропускай.
- Не создавай больше одного объекта на item_index.
- Собери полное название кредитора и полный УИД обращения.
- Обязательно извлеки статус: Отказ, Выдано, Одобрено, На рассмотрении или иной.
- Не путай строку заявления с договором или запросом БКИ.
- Ничего не выдумывай.
...
Верни ТОЛЬКО JSON:
{
  "applications": [{
    ...
    "currency": "RUB или null",
    "uid": "string или null",
    "status": "Отказ|Выдано|Одобрено|На рассмотрении|иное|null",
    "source_page": null,
    "confidence": "high|medium|low",
    "evidence": "до 12 слов",
    "warning": null
